...
# seagull from pinimg,
# mallard from pngall,
# finch from wixmp.com,
# owl from wixmp.com,
# crow from Yopriceville,
# canary from Vecteezy,
# Soil Fertility Dataset from https://www.kaggle.com/datasets/rahuljaiswalonkaggle/soil-fertility-dataset/discussion, 
# circle ring from freepngtransparent.com, 
# listen icon from veryicon,
# dirt pile from pixabay, 

# snowy mountain from Pinterest
# bird logo from vecteezy,
# black circle from vecteezy,
# rocky mountain background from freepnglogos,
# green leaf from freeiconspng

# Bird Dataset, Youtube:
# Seagulls from Mardeo Music
# Mallard from Happy Pony Tail
# Finch from MyBackyardBirding
# Owl from Atmospheric
# Crow from Audibly Sound
# Canary from Relax With Nature


# ChatGPT Prompts:
# how to read outputs from dropdown menu customtkinter
# how to amplify the volume of a wav file in python
# list all customtkinter fonts
...
def openTools():
    ...
    fishCheck.place(relx=0.375, rely=0.55)
...
def save_image_as_png(input, output):
...
def open_file_dialog():
    global previewImage, image_preview
    ...
    if file_path:
        ...
        save_image_as_png(file_path, file+ "preview.png")


        previewIMG = Image.open(file + "preview.png")
        previewImage = CTkImage(previewIMG, size=(300, 280))
        
        image_preview = CTkLabel(
            app,
            image=previewImage,
            text=""
        )

        image_preview.pack(pady=20)
...
previewIMG = Image.open(file + "black_circle.png")
previewImage = CTkImage(previewIMG, size=(300, 280))
